...
- **2026-09-27 (Later, Part 8)**: Major follow-up to the Overview redesign after live user feedback against real parsed-match screenshots. Found and fixed real bugs, not just styling: (1) **Draft data was reading the wrong field entirely** — OpenDota's real field is `picks_bans` (`{is_pick, hero_id, team, order}`, confirmed against `odota_core`'s own schema and sample API responses), not `draft_timings`, which never existed anywhere in any data source; `sync.py` now reads `picks_bans` and stores it under the existing `draft_timings` column. (2) **Autoplay silently never fired** in the embedded playback panel — `MatchMap`'s autoplay guard checked `controlledIsPlaying === undefined`, but `PlaybackSection` always passes a defined boolean, so the condition was permanently false; removed the stale guard. (3) **Hero movement was fundamentally broken, not just unpolished** — `MatchMap.tsx` read `p.pos[minute]`, a field that has never existed in any OpenDota or local-parser response (confirmed by inspecting real parsed-match JSON samples). Traced how Stratz actually gets real hero movement by reading `odota/parser`'s own Java source on GitHub: the raw (non-`/blob`) event stream emits an `"interval"` entry every real game-second per hero — with precise `x`/`y` (the exact same `getPreciseLocation()` transform obs_log/sen_log already use), `life_state`, `gold`, `lh`, `xp`, `level`, `networth` — but `/blob`'s own aggregation code (`CreateParsedDataBlob.java`) explicitly discards interval entries (`case "interval": break;`) before returning its response, which is why they were never in any data we'd been reading. Built `backend/services/position_parser.py`: downloads+decompresses the replay itself (the raw endpoint needs bytes, not a URL, unlike `/blob`), POSTs to the parser's raw endpoint, and extracts a compact per-player `pos_t` series (`{time, x, y, life_state}`, ~1 point/sec) — wired into all three local-parse entry points (auto-parse worker, manual button, `sync.py`'s internal fallback). `MatchMap.tsx` now prefers `pos_t` (true per-second movement) when present, falling back to the previous sparse ward/kill-waypoint interpolation for matches parsed before this change. This backend code is derived from reading `odota/parser`'s actual source, not from a live test (no Docker/parser container available in this sandbox) — needs verification against a real deployed parse. No literal numeric HP/mana field exists even in interval entries (only `life_state`), so continuous HP/mana bars remain a genuinely separate, larger undertaking (would need forking `odota/parser` itself to read additional entity properties) — still deferred. Also reformatted Draft (grouped by team, bigger portraits), Builds (grid-aligned, bigger icons, per-item timestamps), Kill Breakdown (plain colored numbers instead of bars), rebuilt the Towers/Graph/Lane row (real mini-map with tower markers, advantage graph now shows kill-event markers + final gold/XP badges, lane cards show actual hero-icon matchups with a winner tag), reformatted `LiveScoreboardPanel` (fixed a header-overlap bug caused by `noOverflow` letting the items column bleed into the adjacent table; bigger portraits, cleaner row layout), and added explicit W/L badges to the recent-matches strip.
