...
- **2026-09-25 (Late Afternoon)**: Diagnosed and fixed the critical data-loss bug causing blank tables (`-`) on Benchmarks and Performances tabs. `backend/services/sync.py` now aggressively pulls from OpenDota first to secure rich analytics (benchmarks, granular combat logs) and then merges Stratz GraphQL map coordinate data if `data_source` is set to `both`. Added full `Recharts` LineChart implementation to the `LaningTab` mimicking OpenDota's "Last Hits + Denies over time" graph, along with a "WIN" lane trophy indicator.
